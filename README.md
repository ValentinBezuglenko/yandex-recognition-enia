...
Файл `Test_esp32_voice.js` — это тестовый скрипт для эмуляции ESP32 устройства. Он:

1. Создает WebSocket сервер на порту 10000
2. Принимает аудио поток от ESP32
3. Сохраняет полученные аудио данные в двух форматах:
   - `audio.raw` — сырые PCM данные
   - `audio.wav` — аудио в формате WAV с заголовком

Этот файл используется для тестирования и отладки:
- Позволяет проверять обработку аудиопотока
- Помогает отлаживать конвертацию аудио
- Полезен для разработки и тестирования без физического ESP32

Для запуска:
```bash
node Test_esp32_voice.js
```
